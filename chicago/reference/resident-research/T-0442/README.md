# T-0442 — resident research pilot

## Scope

This folder is the durable, human-reviewable handoff for **T-0442**, the pilot
...
- 20 newspaper profile unplaced
- 5 established profile

Of an eligible population of 1362 named non-reconstructed people, 0 had been
reviewed before this pass and 75 after it.

## Results

- **4 corroborated enrichments**
- **7 candidate identities / duplicate leads**, explicitly unasserted
- **64 documented no-corroboration outcomes**
- **0 pending**

Reviewed 2026-08-31. A documented no-find is a research result, not evidence that the
...
- `T-0442_resident_research.csv` — machine-readable Residents table on the shared
  template header (`../cohort_research_template.csv`).
- `T-0442_resident_research_working.xlsx` — working workbook: Residents, Candidates,
  Sources, Search_Log and Summary sheets. Written when `openpyxl` imports.
- `README.md` — this note.

Counts in this package: 75 residents, 7 candidate rows, 12 sources,
150 logged searches.

## The records this is derived from

- `chicago/4d/data/research/residents/pilot_75_cohort.json` — the frozen cohort manifest, re-derived
  by `tools/select_resident_research_pilot.py --gate`.
...
- `chicago/4d/docs/RESEARCH/resident_identity_pilot_75.md` — the pass's narrative dossier.
- `chicago/4d/data/sources/*.json` — the stable source records the rows cite.

`python3 tools/export_resident_research_package.py T-0442 --check` re-derives this folder
and fails if it has drifted from those records; `tools/check.sh` runs it.

## Unresolved

The 7 candidate identities are unresolved by design and are handed to the
T-0487–T-0490 adjudication sequence, not promoted here. The 64 no-corroboration
