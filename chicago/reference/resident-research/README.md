...
Every completed cohort ticket's package, what it holds and what it found. The pilot and
passes 2-3 (T-0442, T-0462, T-0463) carried findings JSON and dossiers but no openable
package until T-0511; `tools/export_resident_research_package.py` writes theirs from the
frozen manifests and the committed review payload, and re-derives them in `check.sh`.

<!-- index:start — generated by tools/export_resident_research_package.py -->

Generated by `python3 tools/export_resident_research_package.py --index`; `--check` fails if it has drifted. Outcome spellings differ between the earlier one-off packagers and are normalized here (`corroborated` and `corroborated_enrichment` are the same outcome, as are `candidate` and `candidate_identity`); a `—` means that package's CSV records no outcome column, not that the count is zero. Row counts are CSV rows, so T-0493's 345 are voter-list readings rather than 345 people.
