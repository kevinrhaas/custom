...
| file | what it is |
| --- | --- |
| `resident_audit_master.csv` | the table, one row per person, 1578 rows and 45 columns |
...
Each line is *how many of the 1578 people carry at least one record of that kind*.

| coverage | of 1578 | % | what counts |
| --- | ---: | ---: | --- |
| identities | **1282** | 81.2% | a name with at least one source id anywhere on the card |
| occupations | **127** | 8.0% | an occupation that is not `none_recorded` |
| household membership | **423** | 26.8% | recorded inside a household of two or more people |
| kinship | **308** | 19.5% | a stated kin relationship (brother, child, daughter, father, husband, mother, sister, son, wife) |
| property / address | **144** | 9.1% | the household resolves a `lives_at` or a `works_at` |
| voter / civic evidence | **302** | 19.1% | a poll book, tax list, muster roll, treaty payment or other public record |
...
| audit result | people | % |
| --- | ---: | ---: |
| `corroborated_across_categories` | 455 | 28.8% |
| `two_or_more_sources_one_category` | 13 | 0.8% |
| `one_source` | 250 | 15.8% |
| `the_letter_lists_alone` | 564 | 35.7% |
| `no_source` | 296 | 18.8% |
...
of the 1578 people. Before T-0733 nothing ruled on any of them, and a conflict
...
| --- | ---: | --- |
| no census linkage | 1531 | no 1840 census row is bridged to this person |
| no address | 1434 | neither `lives_at` nor `works_at` resolves |
| unplaced | 1402 | the household carries division `unplaced`: in the town, on no lot |
| rests on one source | 814 | one source id on the card and no second category to check it against |
| no research row | 742 | no cohort ticket has reviewed this person; the programme reached 836 of 1578 |
| rests on the letter lists alone | 564 | known only from the post office's uncalled-for lists |
| no source of their own | 296 | the collective `household_member` rows — "the rest of the Beaubien household, unnamed" and its two fellows — which are an inferred count of people, not named individuals; the household record carries the sources |
