...
| file | what it is |
| --- | --- |
| `resident_audit_master.csv` | the table, one row per person, 1287 rows and 45 columns |
...
Each line is *how many of the 1287 people carry at least one record of that kind*.

| coverage | of 1287 | % | what counts |
| --- | ---: | ---: | --- |
| identities | **1284** | 99.8% | a name with at least one source id anywhere on the card |
...
| audit result | people | % |
| --- | ---: | ---: |
| `corroborated_across_categories` | 457 | 35.5% |
...
of the 1287 people. Before T-0733 nothing ruled on any of them, and a conflict
...
| --- | ---: | --- |
| no census linkage | 1239 | no 1840 census row is bridged to this person |
| no address | 1211 | neither `lives_at` nor `works_at` resolves |
| unplaced | 1191 | the household carries division `unplaced`: in the town, on no lot |
...
| no research row | 451 | no cohort ticket has reviewed this person; the programme reached 836 of 1287 |
