...
| file | what it is |
| --- | --- |
| `resident_audit_master.csv` | the table, one row per person, 1375 rows and 45 columns |
...
Each line is *how many of the 1375 people carry at least one record of that kind*.

| coverage | of 1375 | % | what counts |
| --- | ---: | ---: | --- |
| identities | **1372** | 99.8% | a name with at least one source id anywhere on the card |
...
| voter / civic evidence | **294** | 21.4% | a poll book, tax list, muster roll, treaty payment or other public record |
...
| `one_source` | 323 | 23.5% |
| `the_letter_lists_alone` | 567 | 41.2% |
| `no_source` | 3 | 0.2% |
...
of the 1375 people. Before T-0733 nothing ruled on any of them, and a conflict
...
| --- | ---: | --- |
| no census linkage | 1326 | no 1840 census row is bridged to this person |
| no address | 1299 | neither `lives_at` nor `works_at` resolves |
| unplaced | 1279 | the household carries division `unplaced`: in the town, on no lot |
...
| no research row | 535 | no cohort ticket has reviewed this person; the programme reached 840 of 1375 |
