...
| file | what it is |
| --- | --- |
| `resident_audit_master.csv` | the table, one row per person, 1586 rows and 45 columns |
...
Each line is *how many of the 1586 people carry at least one record of that kind*.

| coverage | of 1586 | % | what counts |
| --- | ---: | ---: | --- |
| identities | **1279** | 80.6% | a name with at least one source id anywhere on the card |
| occupations | **127** | 8.0% | an occupation that is not `none_recorded` |
| household membership | **434** | 27.4% | recorded inside a household of two or more people |
| kinship | **316** | 19.9% | a stated kin relationship (brother, child, daughter, father, husband, mother, sister, son, wife) |
| property / address | **142** | 9.0% | the household resolves a `lives_at` or a `works_at` |
...
| `one_source` | 247 | 15.6% |
| `the_letter_lists_alone` | 564 | 35.6% |
| `no_source` | 307 | 19.4% |
...
of the 1586 people. Before T-0733 nothing ruled on any of them, and a conflict
...
| --- | ---: | --- |
| no census linkage | 1539 | no 1840 census row is bridged to this person |
...
| no research row | 750 | no cohort ticket has reviewed this person; the programme reached 836 of 1586 |
| rests on the letter lists alone | 564 | known only from the post office's uncalled-for lists |
| no source of their own | 307 | the collective `household_member` rows — "the rest of the Beaubien household, unnamed" and its two fellows — which are an inferred count of people, not named individuals; the household record carries the sources |
