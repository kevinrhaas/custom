...
| file | what it is |
| --- | --- |
| `resident_audit_master.csv` | the table, one row per person, 1370 rows and 41 columns |
...
Each line is *how many of the 1370 people carry at least one record of that kind*.

| coverage | of 1370 | % | what counts |
| --- | ---: | ---: | --- |
| identities | **1367** | 99.8% | a name with at least one source id anywhere on the card |
...
| voter / civic evidence | **284** | 20.7% | a poll book, tax list, muster roll, treaty payment or other public record |
...
| audit result | people | % |
| --- | ---: | ---: |
| `corroborated_across_categories` | 419 | 30.6% |
| `two_or_more_sources_one_category` | 20 | 1.5% |
| `one_source` | 366 | 26.7% |
| `the_letter_lists_alone` | 562 | 41.0% |
| `no_source` | 3 | 0.2% |
...
| --- | ---: | --- |
| no census linkage | 1340 | no 1840 census row is bridged to this person |
| no address | 1294 | neither `lives_at` nor `works_at` resolves |
| unplaced | 1274 | the household carries division `unplaced`: in the town, on no lot |
| rests on one source | 928 | one source id on the card and no second category to check it against |
| rests on the letter lists alone | 562 | known only from the post office's uncalled-for lists |
| no research row | 528 | no cohort ticket has reviewed this person; the programme reached 842 of 1370 |
