...
| file | what it is |
| --- | --- |
| `resident_audit_master.csv` | the table, one row per person, 1281 rows and 45 columns |
...
Each line is *how many of the 1281 people carry at least one record of that kind*.

| coverage | of 1281 | % | what counts |
| --- | ---: | ---: | --- |
| identities | **1278** | 99.8% | a name with at least one source id anywhere on the card |
| occupations | **133** | 10.4% | an occupation that is not `none_recorded` |
...
of the 1281 people. Before T-0733 nothing ruled on any of them, and a conflict
...
| --- | ---: | --- |
| no census linkage | 1234 | no 1840 census row is bridged to this person |
| no address | 1205 | neither `lives_at` nor `works_at` resolves |
| unplaced | 1185 | the household carries division `unplaced`: in the town, on no lot |
| rests on one source | 810 | one source id on the card and no second category to check it against |
| rests on the letter lists alone | 564 | known only from the post office's uncalled-for lists |
| no research row | 445 | no cohort ticket has reviewed this person; the programme reached 836 of 1281 |
