...
| file | what it is |
| --- | --- |
| `resident_audit_master.csv` | the table, one row per person, 1374 rows and 45 columns |
...
Each line is *how many of the 1374 people carry at least one record of that kind*.

| coverage | of 1374 | % | what counts |
| --- | ---: | ---: | --- |
| identities | **1371** | 99.8% | a name with at least one source id anywhere on the card |
...
| voter / civic evidence | **290** | 21.1% | a poll book, tax list, muster roll, treaty payment or other public record |
...
| `two_or_more_sources_one_category` | 14 | 1.0% |
| `one_source` | 324 | 23.6% |
| `the_letter_lists_alone` | 569 | 41.4% |
| `no_source` | 3 | 0.2% |
...
of the 1374 people. Before T-0733 nothing ruled on any of them, and a conflict
...
| --- | ---: | --- |
| no census linkage | 1325 | no 1840 census row is bridged to this person |
| no address | 1298 | neither `lives_at` nor `works_at` resolves |
| unplaced | 1278 | the household carries division `unplaced`: in the town, on no lot |
...
| no research row | 534 | no cohort ticket has reviewed this person; the programme reached 840 of 1374 |
