...
| file | what it is |
| --- | --- |
| `resident_audit_master.csv` | the table, one row per person, 1299 rows and 45 columns |
...
Each line is *how many of the 1299 people carry at least one record of that kind*.

| coverage | of 1299 | % | what counts |
| --- | ---: | ---: | --- |
| identities | **1296** | 99.8% | a name with at least one source id anywhere on the card |
...
| voter / civic evidence | **303** | 23.3% | a poll book, tax list, muster roll, treaty payment or other public record |
...
| audit result | people | % |
| --- | ---: | ---: |
| `corroborated_across_categories` | 454 | 34.9% |
| `two_or_more_sources_one_category` | 13 | 1.0% |
| `one_source` | 262 | 20.2% |
| `the_letter_lists_alone` | 567 | 43.6% |
| `no_source` | 3 | 0.2% |
...
of the 1299 people. Before T-0733 nothing ruled on any of them, and a conflict
...
| --- | ---: | --- |
| no census linkage | 1251 | no 1840 census row is bridged to this person |
| no address | 1223 | neither `lives_at` nor `works_at` resolves |
| unplaced | 1203 | the household carries division `unplaced`: in the town, on no lot |
| rests on one source | 829 | one source id on the card and no second category to check it against |
| rests on the letter lists alone | 567 | known only from the post office's uncalled-for lists |
| no research row | 459 | no cohort ticket has reviewed this person; the programme reached 840 of 1299 |
