...
| file | what it is |
| --- | --- |
| `resident_audit_master.csv` | the table, one row per person, 1306 rows and 45 columns |
...
Each line is *how many of the 1306 people carry at least one record of that kind*.

| coverage | of 1306 | % | what counts |
| --- | ---: | ---: | --- |
| identities | **1303** | 99.8% | a name with at least one source id anywhere on the card |
...
| voter / civic evidence | **301** | 23.0% | a poll book, tax list, muster roll, treaty payment or other public record |
...
| audit result | people | % |
| --- | ---: | ---: |
| `corroborated_across_categories` | 456 | 34.9% |
...
of the 1306 people. Before T-0733 nothing ruled on any of them, and a conflict
...
| --- | ---: | --- |
| no census linkage | 1258 | no 1840 census row is bridged to this person |
| no address | 1230 | neither `lives_at` nor `works_at` resolves |
| unplaced | 1210 | the household carries division `unplaced`: in the town, on no lot |
...
| no research row | 466 | no cohort ticket has reviewed this person; the programme reached 840 of 1306 |
