...
| file | what it is |
| --- | --- |
| `resident_audit_master.csv` | the table, one row per person, 1358 rows and 45 columns |
...
Each line is *how many of the 1358 people carry at least one record of that kind*.

| coverage | of 1358 | % | what counts |
| --- | ---: | ---: | --- |
| identities | **1355** | 99.8% | a name with at least one source id anywhere on the card |
...
| voter / civic evidence | **308** | 22.7% | a poll book, tax list, muster roll, treaty payment or other public record |
...
| `one_source` | 336 | 24.7% |
| `the_letter_lists_alone` | 555 | 40.9% |
| `no_source` | 3 | 0.2% |
...
of the 1358 people. Before T-0733 nothing ruled on any of them, and a conflict
...
| --- | ---: | --- |
| no census linkage | 1309 | no 1840 census row is bridged to this person |
| no address | 1282 | neither `lives_at` nor `works_at` resolves |
| unplaced | 1262 | the household carries division `unplaced`: in the town, on no lot |
| rests on one source | 891 | one source id on the card and no second category to check it against |
| rests on the letter lists alone | 555 | known only from the post office's uncalled-for lists |
| no research row | 517 | no cohort ticket has reviewed this person; the programme reached 841 of 1358 |
