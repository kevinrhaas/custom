...
| file | what it is |
| --- | --- |
| `resident_audit_master.csv` | the table, one row per person, 1379 rows and 45 columns |
...
Each line is *how many of the 1379 people carry at least one record of that kind*.

| coverage | of 1379 | % | what counts |
| --- | ---: | ---: | --- |
| identities | **1376** | 99.8% | a name with at least one source id anywhere on the card |
| occupations | **133** | 9.6% | an occupation that is not `none_recorded` |
...
| audit result | people | % |
| --- | ---: | ---: |
| `corroborated_across_categories` | 464 | 33.6% |
| `two_or_more_sources_one_category` | 15 | 1.1% |
| `one_source` | 325 | 23.6% |
| `the_letter_lists_alone` | 572 | 41.5% |
| `no_source` | 3 | 0.2% |
...
of the 1379 people. Before T-0733 nothing ruled on any of them, and a conflict
...
| --- | ---: | --- |
| no census linkage | 1330 | no 1840 census row is bridged to this person |
| no address | 1303 | neither `lives_at` nor `works_at` resolves |
| unplaced | 1283 | the household carries division `unplaced`: in the town, on no lot |
| rests on one source | 897 | one source id on the card and no second category to check it against |
| rests on the letter lists alone | 572 | known only from the post office's uncalled-for lists |
| no research row | 539 | no cohort ticket has reviewed this person; the programme reached 840 of 1379 |
