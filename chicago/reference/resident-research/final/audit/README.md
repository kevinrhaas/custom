...
| file | what it is |
| --- | --- |
| `resident_audit_master.csv` | the table, one row per person, 1305 rows and 45 columns |
...
Each line is *how many of the 1305 people carry at least one record of that kind*.

| coverage | of 1305 | % | what counts |
| --- | ---: | ---: | --- |
| identities | **1302** | 99.8% | a name with at least one source id anywhere on the card |
...
| property / address | **76** | 5.8% | the household resolves a `lives_at` or a `works_at` |
| voter / civic evidence | **303** | 23.2% | a poll book, tax list, muster roll, treaty payment or other public record |
...
| audit result | people | % |
| --- | ---: | ---: |
| `corroborated_across_categories` | 458 | 35.1% |
| `two_or_more_sources_one_category` | 13 | 1.0% |
| `one_source` | 266 | 20.4% |
| `the_letter_lists_alone` | 565 | 43.3% |
| `no_source` | 3 | 0.2% |
...
of the 1305 people. Before T-0733 nothing ruled on any of them, and a conflict
...
| --- | ---: | --- |
| no census linkage | 1257 | no 1840 census row is bridged to this person |
| no address | 1229 | neither `lives_at` nor `works_at` resolves |
| unplaced | 1209 | the household carries division `unplaced`: in the town, on no lot |
| rests on one source | 831 | one source id on the card and no second category to check it against |
| rests on the letter lists alone | 565 | known only from the post office's uncalled-for lists |
| no research row | 467 | no cohort ticket has reviewed this person; the programme reached 838 of 1305 |
