...
| file | what it is |
| --- | --- |
| `resident_audit_master.csv` | the table, one row per person, 1373 rows and 45 columns |
...
Each line is *how many of the 1373 people carry at least one record of that kind*.

| coverage | of 1373 | % | what counts |
| --- | ---: | ---: | --- |
| identities | **1370** | 99.8% | a name with at least one source id anywhere on the card |
...
| voter / civic evidence | **309** | 22.5% | a poll book, tax list, muster roll, treaty payment or other public record |
...
| audit result | people | % |
| --- | ---: | ---: |
| `corroborated_across_categories` | 468 | 34.1% |
| `two_or_more_sources_one_category` | 22 | 1.6% |
| `one_source` | 312 | 22.7% |
| `the_letter_lists_alone` | 568 | 41.4% |
| `no_source` | 3 | 0.2% |
...
of the 1373 people. Before T-0733 nothing ruled on any of them, and a conflict
...
| rests on one source | 880 | one source id on the card and no second category to check it against |
| rests on the letter lists alone | 568 | known only from the post office's uncalled-for lists |
