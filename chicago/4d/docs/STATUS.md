...
| **Navigation index** | **COMPLETE FOR COMMITTED DATA** — Settings searches all 76 scene structures and all four verified intersections, with aliases and recorded location text; intersection positions are compiled from `data/traces/street_control.json` rather than copied into renderer code. Compass and overview map are independently persistent toggles. |
| **Smoke** | **Navigation is green at both release viewports.** At 390×780 and 1280×800 the live heading, whole-heightfield map, moving marker, all 76 loaded structures, all four verified intersections, search, jump and both settings toggles pass with zero page errors. The full historical suite is 229/251 in this workspace: 20 pre-existing content/evidence assertions no longer match the current compiled dataset, mobile has one residual confidence-frame delta, and the desktop software rasterizer's known 1 fps walk-distance assertion fails. No navigation assertion fails. See § Known weaknesses 0. |
...
| **The platted street module** | **MEASURED on both sheets** — street corridors and widths are committed in `data/traces/vectors/street_corridors_1834.json`, with Lake and Randolph now named from committed control points and re-derived offline by `check_street_module` on every commit. Evidence only: no street geometry is generated yet (S9). |
