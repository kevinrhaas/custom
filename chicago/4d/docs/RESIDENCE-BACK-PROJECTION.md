...
`data/research/directories/residence_back_projection.json`, the liberty is **L221**, and
...
address, and the difference is not marginal: **44 of these 48** are people the 1835 record
gives no trade, whom the business pass had already refused at its clause 1 before its
clause 2 ever looked at them. **Five of the seven placements below are people with no
...
exactly where it is and the later address moves nothing. None of the 48 is in that
...
- **A named house.** `bds Lake House` contains the word *Lake*, and the Lake House is a
  ...
  *Dearborn*, and the fort stands at the river mouth, not on Dearborn Street. Resolving
  ...
  building's name and a street's. Seven entries are refused this way — four public
  houses, the government reservation twice, and the fort — and the reservation is 1835
  ground the town holds and still not a street.
- **A household, named by its head.** `bds Mrs. Post`, `bds John Gray`, `bds Michael
  McDonald` — eight of them, three printed with a title and five bare. Where that person
...
- **`res same`.** Fergus's back-reference to the line above. The crosswalk that fed this
  ...
  typography as evidence about the ground. Four entries.
...
Adams, and moving him to State would place him where it does not say he was. Four
addresses resolve a head street and are then refused on a qualifier — Henry G. Hubbard on
La Salle (Madison), Hibbard Porter and Rebecca Sherman on Clark (Adams; Madison and
Monroe), John P. Simpson on Canal (Adams and Jackson).
...
| outcome | count |
|---|---|
| residence addresses adjudicated | **48** — 34 printed `res`, 14 printed `bds` |
| **placed on a face** | **7**, of which 5 are people with no 1835 trade |
| already better placed (R3) | 0 |
| refused | 41 |

Five faces are reached — **Clark, Lake, Market, Randolph and Washington** — and the seven
are J. H. Collins and Nehemiah King on Lake, Samuel Mills and James M. Morrison on Clark,
L. W. Montgomery on Market, J. B. Jordan on Washington and High Es Jones, who boarded on
Randolph in 1839 at a corner this policy declines to take.
...
record, graded and dated and reversible — and forty-one gained a written refusal where
...
seeing only one. Three of the 48 stand off in the business pass instead (Daniel Elston,
Gholson Kercheval and John Miller carry a real `works_at`), and a stand-off is not a
...
**All 48 are shown, refusals included**, for the reason `residents.js` already gives about
