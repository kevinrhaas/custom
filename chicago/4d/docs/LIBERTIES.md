...
### L181 — Forty willows outside Fort Dearborn's west wall — and the plate's mass is west, not east
**Decision:** `data/flora/plantings/fort_dearborn_wood.json` stands **forty relict black willows,
9.5–11.6 m, on the ground immediately WEST of Fort Dearborn's palisade** — beyond the 12 m of
trodden apron **L174** already claims, wrapping 8 m past each end of that wall and running 45 m
west of it. Every coordinate, every height and the wood's outline are `reconstructed`. Not one
coordinate is authored: the band is derived from `fort_dearborn_palisade`'s own committed
`footprint.polygon` and `placement` in the frame `docs/GLB-CONTRACT.md` fixes, its inner edge is
read out of the apron record's own `apron_width_m`, and `tools/generate_fort_trees.py` re-derives
the file byte for byte in `tools/check.sh`.
**Why:** because **`p4_0` draws a substantial tree mass outside these walls and the render had
none** — T-0044's image-accuracy pass listed it eighth of eight gaps, and `p4_1` draws trees round
the buildings on both banks besides. A garrisoned post standing on bare prairie is what the render
said and it is not what either plate shows.
**THE SIDE IS A CORRECTION, and it is the substance of this entry.** T-0044's row 8 and ticket
...
waterline. And frame-right is **WEST**, settled off the stand rather than off the picture: `p4_0`'s
viewpoint is the north bank at local `1145, 300` looking SOUTH, and the committed
`chicago_lighthouse_1832` — 46.8 m west of the fort's centre — draws to the frame-RIGHT of the fort
in the render from that same stand. The sibling ticket T-0094 was refuted on this same plate for
the same reason; an eye reading of a lithograph is an impression, and this project places nothing
on an impression.
**What is invented, plainly.** Three numbers and everything they generate. (1) **How far west the
wood reaches** — 45 m — which the plate cannot bound at all, because the mass leaves the right edge
of the picture; what it buys is that the REFUSALS rather than the number decide the wood's outline,
and they do: 30 of the 70 dealt grid points are refused, 20 of them for standing in the river's own
bend north-west of the fort, which is what cuts the wood back to the falling ground between the
wall and the water — the ground the plate draws the mass on. (2) **How far it wraps past the wall's
two ends** — 8 m — where the plate makes a statement about bearing and not about plan (the mass
begins 15 px past the stockade's west end, about a metre at the fort's own depth). (3) **How close
the stems stand** — a 7.5 m grid with 1.6 m of jitter — chosen inside the 6–10 m crown width the
zone records for the species, because the plate's canopy CLOSES and a canopy closes when its stems
are inside a crown width of each other.
**What is NOT invented, and it is more than the usual.** The SPECIES is the measurement's and not a
preference. `p4_0`'s crowns stand 127 px above the wall foot: 8.8 m scaled on the fort's committed
53 m footprint, 10.9 m scaled on its committed picket height — the two scales are printed side by
side and never averaged, and they differ by the ±20 % the palisade's own placement note already
carries — plus the 0.70 m the bank falls under the wood, which is derived from the committed
heightfield. Of the three trees `z10_settled_town` records, exactly one is banded low enough to
carry that crown: the relict black willow at 9–14 m, whose own note in that record reads *"Left
along the bank where the landing was cut."* The relict elm (16–24 m) and the relict cottonwood
(18–26 m) are refused in the record's own prose, and `renderers/web/js/trees.js` would refuse the
stem anyway — a 20 m cottonwood here would tower over a fort the plate draws it level with. Every
stem's height is dealt inside the OVERLAP of the species' recorded band and the measured crown, so
it satisfies the renderer's refusal and the picture at once.
**What this deliberately does NOT claim.** The **two-storey frame house with the double gallery**
that stands inside this mass in `p4_0` is not built: that is a structure record, no source this
project holds identifies it, and a building invented to fill a lithograph is a different and much
larger liberty than a tree. Nothing here regrades a metre of the bank or changes the ground
treatment under the wood. And the record makes a claim about ONE side of the fort — the other three
carry the apron and nothing else, because the plate shows a mass on one side and this record does
not extrapolate it round the corner.
**Consequence:** a visitor standing where `p4_0`'s artist stood now sees the fort against a wood on
its river-side flank instead of against empty prairie, and the town's one garrison reads as a post
in a landscape. The cost is forty invented stems, every one of them carrying `reconstructed`, and
a `refused` list in the record that names each of the thirty points the rule declined and why.
**How to resolve:** an 1830s survey of the United States Reservation showing timber; a garrison
return or quartermaster's account of wood cut on the reservation; T-0055's and T-0075's source
records for the plates; or an identification of either plate against a dated original, which would
raise it above the tier-5 standing that lets it drive setting and never a coordinate.
**Ticket:** T-0098, opened by T-0044. Related: **L174** (the trodden apron this wood stands off),
**L179** (the picket point, the sibling refutation on this same plate), **L151** (the dooryard stems, the rule this one is shaped after), **L140** (the fort road that
crosses the same reservation).
**Recorded:** 2026-08-24.
