...
**HOW MANY, AND THE FACT THAT IT MOVES.** This entry was recorded against EIGHTEEN runs and the
record holds ONE. The number is not a decision and never was: it is whatever the rule's five
clauses admit on the day the generator runs, and the committed record is the only place it is
...
seventeen eighteenths of without a word being said, which is why the number now lives on the
record and this entry points at it. **The ground inside the fences is not drawn**, exactly as at the yard and the pound: a
...
would turn one of these from a rule's output into a finding. And holding the Kinzie-view
...
exactly as recorded above. **Revised:** 2026-09-07 (T-0772) — the count. This entry said
*"eighteen"* in five places while the committed record held one, and had done since 2026-08-21.
Nothing about the invention changes: the rule, the treatment, the geometry and the grade are as
...
places and never from a guess: the pound's perimeter and every garden plot on the dooryard record
already CLOSE A RING, and their interiors are that ring, with no new coordinate authored for any of them;
...
the town's Nelson R. Norton — the ship-carpenter who built the Dearborn Street drawbridge — on
rule C2 of `data/residents/card_merge_rulings.json`: the initials agree letter for letter and
he is the one Norton of that initial the town holds. The record is not deleted; it is folded
...
the town gained the card it had been folding away. T-0842 read the page on 2026-09-10 and took it
...
the scanner's digit in `JOHN 8. KINZIE`. T-0993 took it to 484 on the same day, folding
...
`Dr Henry Van der Bogart` — two printings the consolidation had been holding as one man only
because both truncated to the surname `Bogart`, a merge nobody ever ruled on. The minting pass
then wrote a household round the one it had never seen standing on its own, and the reading that
was owed the page was filed as T-0842. It was read on 2026-09-10 and the two ARE one man, under a
new rule C7 that will not fire on a resemblance: it wants the SAME publication shown setting the
particle more than one way for that initial and stem, and here the Democrat sets it three ways.
The 532nd is folded back onto the man it was parted from, and the honorific the notice prints is
recorded as undetermined rather than read either way — the column's left edge is clipped by one
to four characters on almost every line of it, and the surviving `r.` fits `Dr.` as exactly as the
`Mr.` the transcriber supplied.
...
### L231 — The Factor House's fabric is read and its roof is invented, and the fence it is attested to have had is not drawn
**Decision:** `fort_dearborn_us_factors_house` stands 152 m south of Fort Dearborn's pickets,
east of the road, where the 1830 Harrison plan letters **U.S. Factor's House** against three
solid blocks. It is built as **one** of those three — block A, the one the label touches —
20.4 × 7.9 m, two storeys, squared log. **Where it stands, how big it is and how it is turned
is read off the plate; that it is log and that it has two storeys is read off Gurdon Hubbard;
its roof, that roof's pitch, its eave height, its chimney and which of its two long walls is
the front are invented**, and every one of those five is graded `reconstructed`, so a visitor
turning the reconstructed tier off is left with a two-storey log box on measured ground.
**Why:** T-0894 measured this plate and deliberately built nothing, because the sheet gives
arrangement and a ±20 % size and no material, no roof, no opening and no way to say which of
three blocks carries Hubbard's two storeys. T-1036 is that refusal re-examined with the prose
read *alongside* the plate instead of after it, and two of those four gaps close on one clause
— `bk_hub_063`, *"a two-story, squared-log structure"*. That makes this the only building on
the Fort Dearborn reservation whose `construction` is **attested** rather than defaulted: the
two out buildings 130 m north carry `log` as pure invention at **L228**, and the difference
between those two records and this one is a word in a witness.
**What was invented, item by item.** A gable roof; 35° of pitch; a 5.2 m eave, which is two
storeys of about 8 ft 6 in in the clear; one chimney; and `rotation_deg` 320, which faces the
facade north-west onto the road. None is attested for this building or for any building at
this post. The opposite facade choice, 140, gives the identical footprint and moves only the
openings — the long axis itself, bearing 50°, is measured off the drawn bar and is not a
liberty. The chimney count is the archetype's default and the smallest claim available for a
heated two-storey dwelling; a 20 m building could as easily have carried two, and the record
does not guess upward. THE FACADE CHOICE IS NOT IN THE **Covers** LIST BELOW, and that is the
register's rule rather than an oversight: `rotation_deg` lives inside `position`, which is
graded `inferred` as a whole and owes a stated reasoning rather than an admission —
`tools/validate.py` counts only `reconstructed` values as owed one. It is written out here
because a reader of this entry should know it, and the record's own `position.note` states it
where the machinery can see it.
**What is read and is NOT invented:** the position (block A's own ink centre, local ENU
1152.2, 36.4), the size (its principal extents, a ceiling read off a heavy outlined bar whose
fill has broken up on this impression), the bearing, the log, and the two storeys — the last
of those carried one step, from a label that names a building to the block that label touches.
That step is `inferred` and the record writes out the two grounds for it: the label's first
letter stands about 2 m clear of A's north-east end and touches no other block, and A is the
only one of the three a two-storey dwelling-and-store will fit.
**And the split-picket fence is attested and NOT drawn, which is the half of this entry that
had to argue with L229.** Hubbard gives the house *"a neat split-picket fence"* in the same
sentence that gives Mrs Jouett's grave its picket enclosure 50 m away, and **L229** built that
one. Three things separate the cases. FIRST, the plate's silence means something here and
meant nothing there: this sheet DRAWS these three blocks and draws no enclosure round any of
them, while it draws the garrison garden's zigzag fence and John Crafts's Lot's property lines
— at the grave it drew a name and nothing else. SECOND, a grave plot has a size and a house
yard does not: L229 invented 10 × 8 ft around a burial, which is the size a burial is, whereas
the extent of a yard round a 20 m building is bounded by nothing, so the invention would have
been the largest single dimension on this ticket rather than its smallest. THIRD, Hubbard is
describing 1818 — the fence is a government establishment's, and the house had been thirteen
years in private hands by the scene date. **This is not a judgement that there was no fence**;
it is a statement that this project cannot say where it ran.
**And blocks B and C are drawn, unlettered and not built.** 16.6 × 5.1 m set across A 12 m to
the north-west, and 6.7 × 5.0 m 15 m to the south-west. A government factory had a store, a
warehouse, a stable and a smith about it; the sheet says something stood on those footprints
and cannot say which of those things either was, and two invented buildings hung on one label
is the shape `docs/RESEARCH/fort_dearborn.md` refuses for the Well and the Cultivated Field.
Their pixels, sizes and bearings stay in `data/traces/harrison_1830_us_factors_house.json`.
**Consequence:** a visitor walking south from the fort's gate now passes the two small log out
buildings and then meets a long two-storey log house standing alone on the reservation, east
of the road, with open ground round it and no yard fence. The confidence view tells them the
difference: the walls and the storeys read `attested` and `inferred`, the roof above them
reads `reconstructed`, and the ground beside the house is empty because the sheet that drew
the house drew nothing there.
**How to resolve:** a factory return or an Office of Indian Trade inventory for Chicago
1805–1822; a post return naming the building; the 1835 pre-emption case papers over the
reservation; or any second plan of this ground. Any one of those turns the roof into a reading,
and a plat that draws the yard turns the fence into an enclosure record beside the grave's.
Related: **L228** (the two out buildings, the same sheet and the same transform, and the
record of what `log` costs when no witness says it) · **L229** (the grave's fence, built off
the same Hubbard sentence, and the case this entry had to distinguish) · **L226** (the wash
house and the shop, the same plate and the same grade) · **L1** (no figure is drawn) · tickets
**T-1036** (this), **T-0894** (the reading that refused), **T-0881**, **T-0882**, **T-0883**
...
### L234 — Wright draws one road north of the river, and its two ends, its surface and its wear are this project's
**Decision:** `michigan_st_tract_road` in `data/streets/1835.json` runs 262.2 m from
North Water Street to the Michigan St tract's Michigan Street. **The line is Wright's and the
seating is ours.** Four things on that record are not on the sheet: the two end points, the
`track_width_m` of 6.0 m, the `surface` of `light_worn_earth`, and the `traffic` of `light`.
**Why the line is not invented:** the 1834 Wright sheet draws it, as a curved DOUBLE LINE that
cuts diagonally across platted blocks and lot lines — which is what tells a road from a street,
and apart from the fort's road on the reservation it is the only one on that sheet.
`tools/read_michigan_st_tract_road.py` traces both strokes with a ridge follower whose every
parameter is committed, and `--check-sheet` re-runs the identical trace against the raster.
**What is invented, first: the seating.** The road spans two regimes of this sheet's fit. South
of Kinzie the fit is good here — the drawn rule carrying Kinzie's south kerb falls
8.17 m from the committed kerb, inside the registration's own 16.19 m
RMS. North of Kinzie the road is inside the Michigan St tract, which T-1079 seats 38 m north of
where this sheet draws it. So the correction applied is ZERO at the Kinzie crossing and grows
LINEARLY, in the sheet's own northing, to [-14.0, 36.45] m at Michigan
Street — the tract's own committed seating offset, re-derived at this road's north end. **The
linearity is the invention.** Wright's compression is real and its distribution along 100 m of
one road is not recorded anywhere; a step at Kinzie would have been an equally unsourced choice
and a more confident-looking one.
**What is invented, second: the two ends.** The drawn strokes stop inside the Michigan Street
corridor, so the north end is carried 6.0 m on to
`michigan_north_tract`'s centreline; the drawn junction with the river bank falls
8.3 m south of the committed `north_water`, so the south end
is trimmed back on to it. A road that stops 6 m short of the street it meets is a rendering
artefact, not a finding, and both runs are stated on the record and in the trace.
**What is invented, third: the surface.** No source describes this road's surface, its wear or
its traffic, and none names it. The values follow `north_water`'s — the street the road ends on
— because a road that ends on it carried no more traffic than it did. Both grades are
`reconstructed`, the weakest the streets table carries.
**What is NOT taken:** a width. The drawn corridor averages 12.22 m between stroke
centres (sd 2.66 m, 7.08–18.59 m over 36 rows) and that figure is reported
because the sheet measures it, not because the road was built to it. A hand-drawn double line
on a manuscript sheet carries no platted width, and `corridor_width_m` is the mean of the read
rather than a round module the read could be talked into.
**What is NOT claimed:** a name. `name_1835` is descriptive. No source in this repository names
this road, and none names the tract it ends in — see `docs/RESEARCH/michigan_st_tract.md`.
**How to resolve:** a plat or a survey of the ground between Kinzie Street and the tract, which
would replace the linear correction with a measured one; any account of the road north from the
river toward Green Bay that fixes where it left the bank; or a description of its surface.
**Ticket:** T-1080. **Related:** **L60** (the fort road, the other drawn-and-then-reconstructed
line), T-1076 (the reading this seating is hung on), T-1079 (the tract's own seating and its
38.5 m cost).
**Recorded:** 2026-09-12 (T-1080).
