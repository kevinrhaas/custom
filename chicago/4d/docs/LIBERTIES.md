...
is one the compositor himself does not hold to. T-1002 took it to 480 on 2026-09-11, folding three
at once — `beaubien_medore_b`, `clybourn_archibald` and `heacock_russell_e` — under a new rule C10.
All three were found by hand, by T-0961 reading Moses and Kirkland's list of settlers prior to 1830,
and all three differ from the card they fold onto BY ONE LETTER: Madore against Medore, Clybourn
against Clybourne, Russel against Russell. That is the shape the candidate test cannot see from
either side — it buckets the surname exactly, and then asks the forenames for the same whole word —
so none of the three was ever refused or deferred. It was never proposed. C10 does not fold them on
the resemblance: it asks that the folded card's own page carry a FACT BEYOND THE NAME which the
survivor's record holds out of a different document, and here that is a committee of the town's own
board of trustees, a residence on the North Branch beside a beef and pork inspectorship, and a
lawyer's advertisement claiming eight years' residence. AND THE CLASS WAS COUNTED RATHER THAN
GUESSED AT, which is the same answer T-1001 gave at the land register: `tools/measure_card_fold.py`
runs the candidate test again with a fuzzy key and it newly proposes 71 pairs of cards, of which
these three are three. The other 68 stand as two cards each until somebody reads a page for them —
a fold is a card deletion, and this number falls only on evidence. The number this liberty declares
is the number of containers standing, and it will fall again as the remaining rulings are made.
