# T-0510 — Fifteenth resident-research cohort (76)

Status: **manifest frozen 2026-09-03 · research pending** · opened by T-0492

This pass covers the frozen 76-person T-0510 cohort. The ledger at
`data/research/residents/pass_15_findings.json` lists all 76 under `pending` and
`completed_person_ids` is empty, so nothing here changes a resident record yet.
`tools/select_resident_research_pass_15.py --gate` holds the manifest and is wired into
`tools/check.sh`.
...
This cohort holds 74 such reservations.
