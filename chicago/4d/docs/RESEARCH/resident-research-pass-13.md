# T-0508 — Thirteenth resident-research cohort (76)

Status: **manifest frozen 2026-09-03 · research pending** · opened by T-0492

This pass covers the frozen 76-person T-0508 cohort. The ledger at
`data/research/residents/pass_13_findings.json` lists all 76 under `pending` and
`completed_person_ids` is empty, so nothing here changes a resident record yet.
`tools/select_resident_research_pass_13.py --gate` holds the manifest and is wired into
`tools/check.sh`.
...
This cohort holds 75 such reservations.
...
in fixed order. Cohort 13 runs `adams_william_h` … `doc_a_garrett`, and its strata are
