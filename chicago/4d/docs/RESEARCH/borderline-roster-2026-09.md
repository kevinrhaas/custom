...
| `R2_in_window_single_source` | 219 | One appearance inside the window, no card, the ledger withheld it as a single source or on insufficient identity. | Mint a reconstructed resident under the read name. |
...
| `R0_ineligible` | 12913 | Outside Chicago, the Bear Creek marriages (T-1129), the declared `researched_not_resident` names, post-scene arrivals with nothing to back-project from, and names this town already carries. | Never. |
...
| `R2_in_window_single_source/in_window_read_and_withheld` | 219 |
...
### `R2_in_window_single_source` — 219 rows
