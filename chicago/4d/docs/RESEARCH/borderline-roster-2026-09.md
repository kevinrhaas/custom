...
| `R2_in_window_single_source` | 39 | One appearance inside the window, no card, the ledger withheld it as a single source or on insufficient identity. | Mint a reconstructed resident under the read name. |
...
| `R0_ineligible` | 13709 | Outside Chicago, the Bear Creek marriages (T-1129), the declared `researched_not_resident` names, post-scene arrivals with nothing to back-project from, and names this town already carries. | Never. |
...
| `R0_ineligible/owned_by_the_attribute_band` | 462 |
...
| `R2_in_window_single_source/in_window_read_and_withheld` | 39 |
...
### `R2_in_window_single_source` — 39 rows
...
| FAY HARRISON K | 1834-11-04 | `ls0902` | A dated appearance inside the window under a read name, withheld from the town for want of corroboration or identity, and carried on no card. |
