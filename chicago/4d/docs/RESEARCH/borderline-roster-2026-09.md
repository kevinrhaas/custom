...
|---|---:|
| rows | 14544 |
| ledger units considered (every non-`asserted` unit) | 22869 |
...
| `R2_in_window_single_source` | 213 | One appearance inside the window, no card, the ledger withheld it as a single source or on insufficient identity. | Mint a reconstructed resident under the read name. |
...
| `R0_ineligible` | 12864 | Outside Chicago, the Bear Creek marriages (T-1129), the declared `researched_not_resident` names, post-scene arrivals with nothing to back-project from, and names this town already carries. | Never. |
...
| class / rule | rows |
|---|---:|
| `R0_ineligible/already_carried_as_present` | 870 |
| `R0_ineligible/carried_by_the_cards_own_row` | 833 |
| `R0_ineligible/earlier_than_the_window` | 126 |
...
| `R2_in_window_single_source/in_window_read_and_withheld` | 213 |
...
### `R2_in_window_single_source` — 213 rows

| name as read | dated | source | why it was withheld |
|---|---|---|---|
| Rev. Allan B. Freeman | 1833 | `bk_mose1_005` | A dated appearance inside the window under a read name, withheld from the town for want of corroboration or identity, and carried on no card. |
...
| Charles Cleaver | 1833 | `bk_mose1_005` | A dated appearance inside the window under a read name, withheld from the town for want of corroboration or identity, and carried on no card. |
