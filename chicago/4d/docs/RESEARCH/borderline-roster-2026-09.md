...
|---|---:|
| rows | 14897 |
| ledger units considered (every non-`asserted` unit) | 23222 |
...
| `R2_in_window_single_source` | 346 | One appearance inside the window, no card, the ledger withheld it as a single source or on insufficient identity. | Mint a reconstructed resident under the read name. |
...
| `R0_ineligible` | 13100 | Outside Chicago, the Bear Creek marriages (T-1129), the declared `researched_not_resident` names, post-scene arrivals with nothing to back-project from, and names this town already carries. | Never. |
...
| class / rule | rows |
|---|---:|
| `R0_ineligible/already_carried_as_present` | 951 |
| `R0_ineligible/carried_by_the_cards_own_row` | 907 |
| `R0_ineligible/earlier_than_the_window` | 157 |
| `R0_ineligible/later_only_and_not_backprojectable` | 9993 |
...
| `R2_in_window_single_source/in_window_read_and_withheld` | 346 |
...
### `R2_in_window_single_source` — 346 rows
