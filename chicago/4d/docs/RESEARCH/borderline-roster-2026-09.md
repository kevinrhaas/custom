...
|---|---:|
| rows | 14490 |
| ledger units considered (every non-`asserted` unit) | 22862 |
...
| `R2_in_window_single_source` | 184 | One appearance inside the window, no card, the ledger withheld it as a single source or on insufficient identity. | Mint a reconstructed resident under the read name. |
...
| `R0_ineligible` | 12855 | Outside Chicago, the Bear Creek marriages (T-1129), the declared `researched_not_resident` names, post-scene arrivals with nothing to back-project from, and names this town already carries. | Never. |
...
| `R2_in_window_single_source` | `books` | 5 |
...
| class / rule | rows |
|---|---:|
| `R0_ineligible/already_carried_as_present` | 815 |
...
### `R2_in_window_single_source` — 184 rows
...
| Laura Adelaide Sherrill | 1835 | `bk_mose1_012` | A dated appearance inside the window under a read name, withheld from the town for want of corroboration or identity, and carried on no card. |
| Min-ne-mung | 1833 | `bk_fer_024` | A dated appearance inside the window under a read name, withheld from the town for want of corroboration or identity, and carried on no card. |
