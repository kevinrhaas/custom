...
| — | RENDERING | ~~R-BUG5(b)~~ | **SEEN** | **DONE 2026-08-27 (T-0031) — the owner picked route 1 and the belt is back on the skyline.** `main_stem_belt_east` is now DERIVED from the committed `south_water` centreline — offset half a platted corridor south, clipped east at the committed `wells` easting `timberEastLimits()` already hands the near planter — and `tools/derive_timber_belt.py --check` re-derives it on every commit. Census **39 of 39 samples over water, 3.347 m deep → 0 of 136**; the old stub also ended **66.7 m east** of the street it was named for. The side of the street is the one assertion and it is **L182** |
...
on land.** *(Superseded 2026-08-27 by R-BUG5(b) below — the owner ruled and the belt is derived
...
**R-BUG5(b) — where the South Water Street belt stood · DONE 2026-08-27, T-0031, route 1.** The
...
`tools/derive_timber_belt.py` builds the path from `data/streets/1835.json` — the committed
...
**The measurements.** The census went **39 of 39 samples over water, 3.347 m under the surface →
...
sample stands **24–49 m from the water's edge** — inside the 30–74 m gallery `communityAt()` deals
...
at three stands: the belt now wins **19 bearings at the Green Tree anchor** (73–80°, crowns to
36.6 px), **36 at Randolph and Canal** (47–61°, 37.3 px — the stand R-BUG5 reproduced the owner's
screenshot from) and **15 at the forks** (84–89°, 46.9 px). From the `south_water` anchor itself it
...
already stand in this reach**. The far body is not a duplicate of them — it is the same
...
### B-BUG4 — North Water Street's committed centreline runs inside the water mask · **OPEN, T-0226**
...
with sources, so it is a research question rather than a nudge — filed as **T-0226** with the
extent measured.
