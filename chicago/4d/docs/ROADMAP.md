...
| — | KERNEL | **K41** | **CLAIMED 2026-08-16.** AGENTS.md rule 6 and `docs/PROVENANCE.md` both say a `check_required` source "may be cited in text but no asset may be derived from it", and PROVENANCE.md adds *"the validator enforces this"*. The enforcement compares two fields of the SAME source record — `rights_status` against a self-declared `asset_use` — and never asks what the town was built from. **No bake.** Files: `tools/measure_rights_derivation.py` (new), its baseline, `tools/check.sh`, `docs/PROVENANCE.md`, this file, `docs/STATUS.md`, the changelog |
