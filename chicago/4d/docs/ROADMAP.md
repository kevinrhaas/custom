...
| 4 | KERNEL | **K45** | opened by K44: three repairs, none of them a bake — ~~`z08_lakeshore` into `TIMBER_ZONES`~~ (**refuted by K45(a) — it draws nothing; see K45(b)**), ~~a head path in `trees.js`~~ (**CLAIMED 2026-08-16 as K45(c)**), and the 124 herbaceous records a visitor cannot be told the name of. Each is a renderer parcel, so each carries the smoke |
| — | RENDERING | **K45(c)** | **CLAIMED 2026-08-16, in flight** — the head path in `trees.js`: the American basswood's July bloom and the ironwood's fruit, both `cluster_terminal`, both banked by `tools/flora_reach_baseline.json` as `reader-draws-no-head`. Files: `renderers/web/js/trees.js`, `tools/measure_flora_reach.py`, `tools/flora_reach_baseline.json`, docs |
