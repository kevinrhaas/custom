...
PACKAGE_LABELS = {
    ...
    "T-0486": "pass 12", "T-0493": "the 1833-35 voter lists",
}
...
def index_block() -> str:
    note = (
        ...
        "345 are voter-list readings rather than 345 people."
    )
...
def write_index() -> int:
    ...
    print("index written: %d packages" % len(PACKAGES_OWED))
    return 0
...
def check(tickets: list[str]) -> int:
    ...
    if problems:
        for line in problems:
            print("FAIL  " + line)
        return 1
    print("resident-research packages ok — %d folders present, %d re-derived from source, "
          "index current" % (len(PACKAGES_OWED), len(tickets)))
    return 0
