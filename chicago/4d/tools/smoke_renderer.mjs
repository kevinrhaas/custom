 ...
 *   draw calls under budget ..... the batch strategy is doing its job, at the budget
 *                                 main.js sets — RAISED 80 -> 110 by T-0064, argued there
...
for (const [label, viewport, touch] of [
  ...
  if (ready) {
    ...
    // --- budgets ------------------------------------------------------------
    //
    // THE NUMBER IS THE PAGE'S, NOT THIS FILE'S, and that matters more since
    // T-0064 raised it. `main.js` sets `BUDGET.drawCalls` and argues the value at
    // its definition: 80 was set when the town was one batched mesh plus a few
    // whole-layer meshes, and three layers have since split into culling-sized
    // chunks on purpose (T-0119, T-0067, T-0064), trading calls for triangles.
    // 110 is this parcel's worst measured reading plus about a sixth. Reading the
    // budget off the page rather than restating it here is what keeps this check
    // honest through a re-budget: it asserts the scene is inside the ceiling the
    // scene declares, and a silent raise would show up in the printed number.
    ...
    check(`${label}: the scene declares a draw-call budget it argues for`,
      stats.budget.drawCalls === 110,
      `budget ${stats.budget.drawCalls} calls (raised from 80 by T-0064, argued in main.js)`);
