...
def plain_fragment(name: str) -> str:
    ...
    `plain_fragment("B. S. Morris")` -> `morris_b_s`; T-0594.
    """
    fam = surname(name)
    given: list[str] = []
    dropped = False
...
def minted_by(path, doc: dict, pass_name: str, legacy_prefix: str) -> bool:
    ...
    legacy filename prefix or by its `source_pass` field (T-0594)."""
...
def town_family_names(docs: dict, index: dict, skip_prefix: str | None = PREFIX) -> set[str]:
    ...
    T-0594: `skip_prefix` still names this pass's legacy prefix, but the test also
...
def gate_problems(docs: dict, index: dict, structure_text: dict) -> list[str]:
    ...
    # T-0594: a letter-list person's id is no longer always `ll_...` or found
