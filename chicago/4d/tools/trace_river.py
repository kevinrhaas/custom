...
# The old square window's south edge, in local ENU metres. Everything north of
# this line was decided by the window that produced the committed trace, so the
# T-0219 extension must reproduce it — that is what makes a widened window an
# EXTENSION rather than a re-trace of the whole river.
CHECK_NORTH_LINE_M = -400.0
# The trace's own stated accuracy (docs: "+/- 20 m on the ground"). A vertex that
# moves further than this has not been carried by the extension, it has been
# re-decided by it, and the extension is then not the change it claims to be.
CHECK_NORTH_TOL_M = 20.0


def check_north(river_fc, hydro_fc, origin_e, origin_n) -> int:
    """Hold the extended window to the square window's own answer.

    Both files are written in UTM (their own provenance says so: "ENU metres used
    by the scene are these minus data/datum.json origin_utm_e / origin_utm_n"), so
    every coordinate is shifted to local ENU before the line is applied. Getting
    that wrong is not a small error and it does not fail loudly: a UTM northing of
    4.6 million passes `y >= -400` trivially, so the filter selects the whole
    river, the newly extended tip is compared against a bank that stops 200 m
    north of it, and the check reports the extension it was written to ignore.

    `--check` diffs the whole file and says DIFF the moment the window changes,
    which is true and useless: the south IS supposed to change. What has to be
    proved is the other half — that widening the window on the south side alone
    left the north alone. The region ORIGIN is unchanged by construction, so
    every local-percentile block in PARAMS lands on the same pixels; this
    measures whether that held all the way through the morphology.

    Every re-traced vertex north of CHECK_NORTH_LINE_M is measured to the nearest
    point on the COMMITTED POLYLINE, and the largest such distance is reported.

    Three things that metric is chosen over, each of which reports a number that
    is not a movement:

      * index-wise comparison — the rings are re-walked from a fixed start, but a
        longer ring renumbers, so a rotation would read as a displacement;
      * nearest committed VERTEX — that measures how finely the old ring happened
        to be sampled, not where it ran. Measured here: it reports 204 m on a
        bank the segment distance puts under 3, because the committed ring
        carries one vertex where the re-trace carries several;
      * distance to the whole committed file — the old ring is CLOSED across the
        old window's south edge by a cut line that the extension correctly
        deletes, and matching to that line would score the deletion as agreement.
        Only segments with an endpoint north of the line are considered.
    """
    import math
    ...
    def rings(fc):
        ...
        for feat in fc["features"]:
            ...
            if geom["type"] == "LineString":
                ...
                polys = [[c] for c in coords]
            for poly in polys:
                for ring in poly:
                    out.append([(float(x) - origin_e, float(y) - origin_n)
                                for x, y in ring])
        return out
    ...
    for name in ("river.geojson", "hydrology.geojson"):
        path = OUT_DIR / name
        if not path.exists():
            die(f"{path.relative_to(ROOT)} is not committed; nothing to check against")
        committed[name] = rings(json.loads(path.read_text()))

    worst_overall = 0.0
    checked = 0
    for name, fc in (("river.geojson", river_fc), ("hydrology.geojson", hydro_fc)):
        new_rings, old_rings = rings(fc), committed[name]
        segs = [(a, b) for r in old_rings for a, b in zip(r, r[1:])
                if a[1] >= CHECK_NORTH_LINE_M or b[1] >= CHECK_NORTH_LINE_M]
        if not segs:
            die(f"{name}: the committed geometry has no segment north of "
                f"{CHECK_NORTH_LINE_M:g} m, so there is nothing to hold the "
                f"extension to. Refusing to report a pass on an empty comparison.")
        worst, worst_at = 0.0, None
        for ring in new_rings:
            for x, y in ring:
                if y < CHECK_NORTH_LINE_M:
                    continue
                checked += 1
        ...
        where = f" at E {worst_at[0]:+.1f} / N {worst_at[1]:+.1f}" if worst_at else ""
        print(f"{name}: {len(segs)} committed segments north of "
              f"{CHECK_NORTH_LINE_M:g} m; re-traced worst departure {worst:.2f} m{where}")

    if not checked:
        die("no re-traced vertex lies north of the line; the comparison is empty")
    print(f"checked {checked} re-traced vertices; worst {worst_overall:.2f} m "
          f"against a {CHECK_NORTH_TOL_M:g} m tolerance")
    if worst_overall > CHECK_NORTH_TOL_M:
        print(f"FAIL the window extension moved the trace north of "
              f"{CHECK_NORTH_LINE_M:g} m by {worst_overall:.2f} m")
        return 1
    print("OK   the extension carried the north unchanged")
    return 0
...
def main() -> int:
    ...
    ap.add_argument("--check-north", action="store_true",
                    help="re-trace and measure the result NORTH of the old n_min against "
                         "the committed geometry; the T-0219 window extension must not "
                         "move anything the square window already decided")
    ...
    if args.check_north:
        return check_north(river_fc, hydro_fc,
                           datum["origin_utm_e"], datum["origin_utm_n"])
