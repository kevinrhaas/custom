...
def rebuild_index(index, docs, stats):
    # The rows and the derived counts belong to ONE derivation now (T-0715). This
    # function used to state them itself, starting each row from the row already
    # committed (`row=dict(old.get(hid) or {})`) - which carried a flag another
    # pass had written straight through, stale and unexamined.
    index=rebuild_resident_index.rebuild(index,docs)
    ...
    counts["reconstructed_removed_in_2026_09_02_synthesis"]=stats["removed_people"]
    index["counts"]=counts
