...
def make_record(seq: int, family: str, e: float, n: float, row: dict,
    ...
    reconstruction = {
        "status": "recommended_anonymous", "family": family, "district": "south",
    ...
    location = f"Anonymous recommended roof in the South Division block between {blocks}, south of Lake Street"
    function = function_for(family)
    return {
        "id": sid,
        "name": f"Recommended {family} {function} #{seq:03d}",
        ...
        "phases": [{
            ...
            "change_note": "Recommended anonymous July 1835 infill. It may later be replaced by a named, better-evidenced roof through an explicit inventory substitution."
        }],
...
def main() -> int:
    ...
    if drift:
        print("RECOMMENDED INFILL DRIFT")
        for item in drift:
            print(f"  - {item}")
        return 1
    mode = "verified" if args.check else "generated"
    print(f"{mode} {len(records)} recommended anonymous South Division records (40 principal, 8 ancillary)")
    return 0
