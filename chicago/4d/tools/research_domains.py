...
# RECORD-ID SCOPE (T-1338). `tools/research_spend_ledger.py` keys every reading unit on its
# row id, and a container whose ids repeat across its own files has to say which of two
# things it means by that -- the same subject read twice, or two unrelated rows that happen
# to sit at the same position in different files. See the manifest's own `_doc`. Declaring
# it is what the ledger's `record_id_scope_faults` requires; an undeclared repeat is a fault
# there rather than a silent reading, because the silent reading was `global` and it closed
# 145 units as asserted off a newspaper claim number.
def declared_units(**scope) -> list[dict]:
    """`DECLARED_UNITS`, with a record-id scope declared on its single pattern."""
    pattern = dict(DECLARED_UNITS[0])
    pattern.update(scope)
    return [pattern]


PERSON_IDS_ARE_GLOBAL = (
    "A person id is the person. The pass cohorts are cohorts of one town and 225 people are "
    "read by more than one of them, so the bare id is the key and must stay so.")

CLAIM_IDS_ARE_FILE_LOCAL = (
    "A claim id is a position in the extraction of ONE issue; 55 issue files each print "
    "c001, and the ledger key is therefore the file's stem and the id joined by `#`.")
...
DOMAINS = {
    "civic": {
        ...
        "ledger_units": declared_units(
            ...
            record_ids_note="Same numbering as the papers', and this domain's c013 collided "
                            "with the Democrat's across the two of them."),
    },
    "census_1830": {
    ...
    "census_1840": {
        ...
        "ledger_units": declared_units(
            file_local_containers=["records"],
            record_ids_note="These rows carry no id at all, so the key falls back to the "
                            "file-relative JSON pointer /records/0, which every sheet has."),
    },
    "church": {
    ...
    "newspapers": {
        ...
        "ledger_units": [{
            ...
            "id": "row.id", "name_fields": ["normalized", "quote"],
            "file_local_containers": ["claims"],
            "record_ids_note": CLAIM_IDS_ARE_FILE_LOCAL,
        }],
    },
    "residents": {
        ...
        "ledger_units": [
            {"glob": "pass_*_cohort.json", "containers": ["people"],
             "id": "row.person_id",
             "global_containers": ["people"],
             "record_ids_note": PERSON_IDS_ARE_GLOBAL},
...
def build(research: Path = RESEARCH, sources: Path = SOURCES, quiet: bool = False) -> int:
    ...
    manifest = {
        ...
        "_doc": "GENERATED by tools/research_domains.py --build. Every committed "
                ...
                "\n\nRECORD-ID SCOPE (T-1338). A ledger unit's key in "
                "tools/research_spend_ledger.py is its row id, and two corpora here reuse a "
                "row id across their own files while meaning opposite things by it: a "
                "residents pass cohort names the same man in four passes, and a newspaper "
                "issue numbers its claims from c001 with no reference to any other issue. "
                "Nothing in the id tells them apart, so each container whose ids repeat says "
                "which it is. `file_local_containers` means the key is the source file's "
                "stem and the id joined by `#` \u2014 `chicago_democrat_1835_08_19#c007`, "
                "the form the resident cards' own notes already wrote \u2014 and a card must "
                "name that whole form to spend such a unit. `global_containers` means the id "
                "is a name the whole corpus agrees on and the bare id is the key. An "
                "undeclared repeat is a fault: taking it for global silently is what closed "
                "145 units as asserted off three cards that had each cited exactly one "
                "claim.",
