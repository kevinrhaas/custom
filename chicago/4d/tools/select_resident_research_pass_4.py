...
def derive() -> dict:
    ...
    pass3_ids = set(PASS3_ESTABLISHED_IDS) | set(PASS3_PRESENT_LETTER_IDS) | set(PASS3_UNCERTAIN_LETTER_IDS)
    ...
    people += [member(index, pid, "letter_list_only",
                      "Scene-date postal return selected for identity, OCR/variant and duplicate testing.")
               for pid in PRESENT_LETTER_IDS]
    people += [member(index, pid, "letter_list_only",
                      "Earlier or uncertain postal return selected for identity, migration and regional-disambiguation testing.")
    ...
    eligible = sum(p.get("grade") != "reconstructed"
                   for h in households for p in h.get("persons", []))
    return {
        "_doc": "T-0478's reproducible fourth 75-person research cohort; selection is not evidence about a person.",
        "version": 1,
        "ticket": "T-0478",
