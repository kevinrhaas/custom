...
PASS3_COHORT = ROOT / "data/research/residents/pass_03_75_cohort.json"
PASS3_FINDINGS = ROOT / "data/research/residents/pass_03_findings.json"
...
def derive() -> dict:
    prior = load(OUT)
    cohort, findings = load(PASS3_COHORT), load(PASS3_FINDINGS)
    baseline = prior.get("reviews", [])[:150]
    tickets = prior.get("tickets", [prior.get("ticket")])
    if len(baseline) != 150 or tickets[:2] != ["T-0442", "T-0462"]:
        raise SystemExit("expected the committed T-0442/T-0462 150-review baseline")
    ...
    if extras := set(findings["overrides"]) - cohort_ids:
        raise SystemExit(f"pass-three findings outside cohort: {sorted(extras)}")
    ...
    pass_counts = []
    for ticket, start in (("T-0442", 0), ("T-0462", 75), ("T-0463", 150)):
        review_pass = reviews[start:start + 75]
        pass_counts.append({"ticket": ticket, "size": 75,
                            "counts": {k: sum(r["outcome"] == k for r in review_pass) for k in outcomes}})
    return {"_doc": "T-0442, T-0462, and T-0463 public resident research reviews. Candidate biographies are leads, not asserted resident facts.",
        "version": 3, "ticket": "T-0463", "tickets": ["T-0442", "T-0462", "T-0463"], "scene_date": "1835-07-01",
        ...
        "counts": counts, "passes": pass_counts, "reviews": reviews}

def main() -> int:
    parser = argparse.ArgumentParser(); parser.add_argument("--gate", action="store_true"); args = parser.parse_args()
    doc = derive()
    if len(doc["reviews"]) != 225 or sum(doc["counts"].values()) != 225:
        raise SystemExit("resident research must compile exactly 225 non-overlapping reviews")
