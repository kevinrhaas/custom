...
def derive() -> dict:
    ...
    if len(cohort_ids) != 75:
        raise SystemExit(f"pass-five cohort must contain 75 unique people, got {len(cohort_ids)}")
    if set(findings.get("pending_person_ids", [])):
        raise SystemExit("pass-five findings still contain pending people")
    if set(findings.get("completed_person_ids", [])) != cohort_ids:
        raise SystemExit("pass-five completed_person_ids must equal the fixed cohort")
    ...
    if pass_counts[-1]["counts"] != expected_pass5:
        raise SystemExit(f"pass-five outcome census changed: {pass_counts[-1]['counts']}")

    expected_total = {
        "corroborated_enrichment": 74,
        "candidate_identity": 61,
        "no_corroboration": 240,
    }
    if counts != expected_total:
        raise SystemExit(f"cumulative 375-person outcome census changed: {counts}")

    eligible = cohort["population_frame"].get(
        "technical_nonreconstructed_entries",
        prior.get("eligible_real_named_people", 848),
    )
    return {
