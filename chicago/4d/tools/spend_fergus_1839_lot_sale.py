...
T-0681 lands." This is that pass, and it pays the whole eleven rather than the visible
three: the other eight cards cite the volume only because Fergus' 1837 poll or its 1839 city
register happened to name the same household, which makes the second hop read them as
written while the lot sale itself had reached nobody.
...
THE FOUR RULES, unchanged from passes 1-3 because the defect they guard against is the same.
  ...
  1. ONLY WHAT THE CROSSWALK ALREADY DECLARED. This pass re-adjudicates nothing. It reads
     `residents.matches` and writes those eleven. The 26 `contested` and 3 `ambiguous` rows
     are rivals still standing; the 89 surname-only rows are refusals; and the voter,
...
def ledger_doc() -> dict:
    ...
    return {
        ...
        "_doc": (
            ...
            "of the Fort Dearborn Addition lot-sale crosswalk's eleven resident matches "
...
def apply_to_person(person: dict, row: dict) -> bool:
    ...
    if SOURCE_ID not in existing:
        # SORTED IF IT ARRIVED SORTED. Nine of these eleven cards are minted by
