...
def crosswalk(records, people):
    """Scene year only. Everything the surname-and-initials rule reaches is a CANDIDATE;
    a candidate is promoted to a merge only where a second attribute agrees too."""
    by_surname = {}
    for p in people:
    ...
    entries = []
    for rec in records:
        if not str(rec["describes_date"]).startswith("1835"):
            continue
        out = {
            "record_id": rec["id"],
            "as_read": rec["as_read"],
...
def build(root: Path = ROOT, write: bool = True):
    ...
    pass_doc = {
        ...
        "_doc": "GENERATED by tools/read_st_cyr_register.py --build. The mechanical pass, "
                ...
                "was concluded. SCENE YEAR ONLY: the 26 readings dated 1835 on either "
                "page, against the named persons of data/residents/. An `unmatched` row "
                "is a person no resident record carries a surname for and is the ordinary "
                "case; a `refusal` is a surname that folds equal with no forename that "
                "agrees; a `candidate` is what the rule reaches, and a candidate is not a "
                "merge. The adjudications are in crosswalk.json.",
        "generated_by": "tools/read_st_cyr_register.py --build",
        "pass": TICKET,
        ...
        "why_so_few": "Six marriages and four deaths is the whole of the scene year here, "
                      "26 named readings in all, and the town's resident file is mostly "
                      ...
                      "is the honest result and not a failure of the rule.",
        "entries": walk,
    }
