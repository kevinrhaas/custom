...
def plain_fragment(name: str) -> str:
    ...
    them in. This is the shape a NEW household's id and person id take (T-0592):
...
# T-0592: a household minted from here on gets a PLAIN id (`plain_fragment`
...
def household_id(cand_name: str, prefix: str, pass_name: str, docs: dict,
    ...
    plain id if a run after T-0592 already minted them under that — and mint a
...
def town_family_names(docs: dict, index: dict, skip=MINTED_PASSES) -> set[str]:
    ...
    T-0592 is recognized the same as a legacy-prefixed one. See MINTED_PASSES
...
def record(cand: dict, gaz: dict, docs: dict, taken_ids: set[str]) -> dict:
    ...
    if hid != legacy_id:
        # A genuinely new mint (T-0592): the plain id carries no pass of its own,
