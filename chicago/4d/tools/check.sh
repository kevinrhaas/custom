...
# lists — a clean bill of health that was an artefact of the instrument's altitude. T-0839's
# card merges then landed and settled four of the contested names, so the crosswalk names 15.
# The step is deliberately not numbered: this pass writes whatever the crosswalk declares,
# and is gated in the same two directions as its three predecessors:
...
step "…and the Fort Dearborn lot sale is on every card it names" \
