...
**From T-0211, which is what this cost.** `chicago-4d-check.yml` runs `check.sh` and nothing
...
machine (T-0211 has the frame timings). A fourth agent was simultaneously re-deriving the
...
the conditions it was taken under — T-0211's readings differ by a factor of twenty on the same
tree.

**Why it is not built in T-0211's PR:** it edits `.github/workflows/`, which AGENTS.md
...
for every agent, so two concurrent runs on the shared box collide with `EADDRINUSE` (T-0211's
first command did). The deeper version of the same problem is that a dozen simultaneous
SwiftShader browsers ARE the load T-0211 measured — 71-115 Chromium processes, load average
