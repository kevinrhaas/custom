---
id: T-0484
title: Research tenth 75-person real-resident cohort
state: claimed
epic: META
requested_by: owner
seen: true
...
closed: null
pr: null
claimed_by: chatgpt-resident-agent-sol56c 9/2/2026, 4:27 PM CT
blocked_on: null
needs_bake: false
---

Continue the resident-identity research programme with a fixed, non-overlapping 75-person cohort of real named residents. This queue ticket is the tenth continuation after the pilot and passes 2–5 (375 reserved IDs). The frozen manifest for this ticket contains 31 scene-date letter-list names and 44 earlier/uncertain letter-list names; no named non-letter-list profiles remain in the eligible pool.
...
Repository handoff is part of the ticket. Commit a cohort research package under `chicago/reference/resident-research/T-0484/` containing at minimum a human-reviewable XLSX working table, a machine-readable CSV export, and a README/method note describing sources, fields, confidence rules, ambiguities and limitations. The package must retain one row per manifest person plus any candidate identities considered, with stable person IDs, normalized/transcribed names, outcome, proposed facts, evidence for/against, source IDs/URLs, source tier, query/access date and notes. Commit the filled JSON findings ledger and every new or updated source record alongside it. Findings that exist only in chat, browser notes or an unpushed working tree do not count.
...
- The completion PR targets `dev`, records the cumulative reviewed total (750 after this cohort), and closes this ticket only after all research artifacts are committed. The already-merged planning scaffold reserves the cohort; it is not completion.
