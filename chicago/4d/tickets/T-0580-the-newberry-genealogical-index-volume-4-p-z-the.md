...
Internet Archive file, its size and its sha256; fetch it to a scratch path and run
    ...
    python3 tools/read_newberry_index.py --extract --volume 4 --pdf <path>
...
- `text/vol_04_locality_cards.txt` committed, and MANIFEST carrying the volume's sha256,
  its crop boxes and the sha256 of every intermediate, so the extraction is reproducible.
- `records/entries_vol_04.json` in the T-0492 records shape, every record
