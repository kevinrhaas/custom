...
Piece 2 of 2 of **T-1144 — Converge the resident layer after the standing truth tickets: zero synthesis and mint drift, no false Chicago resident, and no 1835 claim above its dated evidence**, split because the parent needed more than one run's demonstration to be done. The parent keeps the full ask and its links; this ticket owns one slice of it.

**Acceptance:** (state it before working — one demonstration, never weakened to pass)

1. The letter-list mint's drift is **measured and named**: the 798 files behind
   T-0691/T-0660 that T-1222 reads, counted, with what moved and why the mint and the
   committed list disagree. A number, not "it drifts".
2. The one-letter-apart identity rule is written down as a rule a crosswalk can apply —
   naming both spellings verbatim, the way this project's every other merge rule does — or
   it is ruled impossible here and the reason is stated. Joining two names by eye is the
   manufacturing of identity this ticket exists to refuse.
3. `identity_open_one_letter_apart` in `data/research/spend_rulings.json` points at this
   ticket and its units are spent, refused, or handed to live work by the time this closes.
   That line is why this child exists: under the T-1237 rule a unit cannot defer to spent
   work, and T-1144 is now a split parent.
4. It agrees with **T-1222** rather than racing it. T-1222 owns the 798-file drift; this
   ticket owns what the resident layer does about it. If the two disagree about a file,
   that disagreement is stated, not silently resolved in one of them.
5. No resident is minted, renamed or folded to make a rule come out even.
