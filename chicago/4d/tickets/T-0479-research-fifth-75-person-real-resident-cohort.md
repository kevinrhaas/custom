...
PR #625 was the initial tranche, not completion: 66 postal-list members remained explicitly pending. The completion work must also commit a cohort research package under `chicago/reference/resident-research/T-0479/` following `chicago/reference/resident-research/README.md`: a human-reviewable XLSX workbook, a machine-readable CSV export, and a README/method note, plus the completed JSON findings ledger and all new/updated source records. Findings that exist only in chat, browser notes or an unpushed working tree do not count.
