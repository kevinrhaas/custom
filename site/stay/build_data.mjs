 ...
 *
 * Reads the per-region research files (raw-<region>.json), normalises them into
 * one factual dataset and writes js/data.js. Facts only: no scoring, no tiers,
 * no "best value" — all of that lives in js/app.js so the data file stays
...
console.log(`wrote js/data.js — ${properties.length} properties`);
