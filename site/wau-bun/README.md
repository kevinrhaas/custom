...
- Each scene reads four ways: **Summary**, the **Modern** text in full, the
  **Dark** retelling, or the **1856** original in full — all four, for every
  scene in every part.
...
- `js/data-dark-part<N>.js` holds the **Dark** retelling: the same events in a
  modern horror-suspense voice. The book earns it — it contains a massacre, a
  captivity, a starvation spring, a woman held under the water of Lake Michigan
  to keep her out of a fight — and the 1856 prose reports all of it in a
  drawing-room register. This mode restores the dread those events carried.

  It is **original prose written for this app**, not a translation and not a
  pastiche of anyone's book: every event, person, place, date, number and line
  of dialogue in the source survives, nothing is invented, and Juliette's own
  judgements about how the Ho-Chunk and Sauk were treated carry over at full
  force. The passages live one per scene in `wau-bun/dark/<scene-id>.txt`;
  `wau-bun/dark/BRIEF.md` is the specification they were written against.
