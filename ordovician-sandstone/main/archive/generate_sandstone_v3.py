...
def build_hollow_mesh(new_rings, target_height, wall_thickness, solid_base=0.0):
    ...
    Structure:
      ...
      - If solid_base > 0: solid bottom disc (fan) at z=0 + inner wall
        starts at solid_base with an annular lip connecting to outer wall
...
def main():
    ...
    elif args.hollow or args.base is not None:
    ...
    if is_hollow:
        all_points, faces = build_hollow_mesh(
            new_rings, target_height, wall_thickness, solid_base)
    else:
    ...
    if args.output:
        ...
        if is_hollow:
            ...
            if solid_base > 0:
                # Use full precision for fractional bases
