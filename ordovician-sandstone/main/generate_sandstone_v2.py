...
The printable release assembly also includes the proven 80 mm lamp-attach
connector in the lower 9.46 mm of the shell.  Its thread geometry is imported
unchanged so the shade mates with files/base/base-115mm-v3.stl.
...
DEFAULT_CONNECTOR = SCRIPT_DIR.parent / "files" / "connect" / "80mm-lamp-attach-base.stl"
DEFAULT_OUTPUT_DIR = SCRIPT_DIR.parent / "files" / "lamp" / "v2" / "connected"
...
def read_binary_stl(path: Path) -> Mesh:
    """Read a binary STL and weld vertices with identical float32 coordinates."""
    ...
    if len(data) != expected_size:
        raise ValueError(
            f"Expected binary STL size {expected_size}, found {len(data)}: {path}"
        )
    ...
    for triangle_index in range(triangle_count):
        ...
        for start in (3, 6, 9):
            ...
            if point not in point_indices:
                ...
                points.append(point)
            face.append(point_indices[point])
        faces.append((face[0], face[1], face[2]))
    return Mesh(points, faces)


def combine_meshes(meshes: Sequence[Mesh]) -> Mesh:
    """Combine positioned meshes without altering their source geometry."""
    points: list[Point] = []
    faces: list[Face] = []
    for mesh in meshes:
        offset = len(points)
...
def mesh_bounds(mesh: Mesh) -> tuple[list[float], list[float]]:
...
def write_scad(
    ...
    connector_mesh: Mesh | None = None,
) -> None:
    with path.open("w", encoding="utf-8") as handle:
        handle.write(
            "// Illinois Sandstone Lamp v2\n"
            "// Generated by generate_sandstone_v2.py\n"
            "// Watertight lamp body; source rings preserved; seamless base transition.\n"
            f"// height={parameters.height} layers={parameters.layers} wall={parameters.wall} "
            f"base={parameters.base} hole={parameters.hole}\n"
            f"// connector={parameters.connector or 'none'}\n\n"
            "/* [Height Fine-Tuning] */\n"
            "fine_tune_scale = 1.0;  // [0.90:0.001:1.10]\n\n"
            "scale([1, 1, fine_tune_scale])\n"
        )
        if connector_mesh is None:
            write_scad_polyhedron(handle, mesh)
        else:
            handle.write("union() {\n")
            write_scad_polyhedron(handle, mesh, "  ")
            write_scad_polyhedron(handle, connector_mesh, "  ")
            handle.write("}\n")


def mesh_object_xml(object_id: int, mesh: Mesh, name: str) -> Iterable[str]:
    yield f'    <object id="{object_id}" type="model" name="{name}">\n      <mesh>\n        <vertices>\n'
...
def model_xml(mesh: Mesh, connector_mesh: Mesh | None = None) -> Iterable[str]:
    ...
    yield '  <resources>\n'
    yield from mesh_object_xml(1, mesh, "Sandstone lamp body")
    if connector_mesh is None:
        yield '  </resources>\n'
        yield '  <build>\n    <item objectid="1" printable="1"/>\n  </build>\n</model>\n'
    else:
        yield from mesh_object_xml(2, connector_mesh, "80 mm threaded lamp connector")
        yield (
            '    <object id="3" type="model" name="Connected lamp assembly">\n'
            '      <components>\n'
...
def write_3mf(path: Path, mesh: Mesh, connector_mesh: Mesh | None = None) -> None:
...
"""
    with ZipFile(path, "w", compression=ZIP_DEFLATED, compresslevel=6) as archive:
        ...
        archive.writestr(
            "3D/3dmodel.model", "".join(model_xml(mesh, connector_mesh))
        )
...
def write_report(
    ...
    assembly_metadata: dict[str, object] | None = None,
) -> None:
    payload = {
...
def build_argument_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="outer source/resampling rings; defaults proportionally from height",
    )
    ...
    parser.add_argument(
        ...
        help="80 mm connector STL embedded unchanged in the lower base",
    )
    parser.add_argument(
        "--no-connector",
        action="store_true",
        help="generate only the watertight lamp body",
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if layers is None:
        layers = max(
            len(source_rings),
            int(round(len(source_rings) * args.height / 120.0)),
        )
    ...
    connector_mesh: Mesh | None = None
    connector_display: str | None = None
    assembly_metadata: dict[str, object] | None = None
    if not args.no_connector:
        connector_path = args.connector.resolve()
        connector_mesh = read_binary_stl(connector_path)
        connector_min, connector_max = mesh_bounds(connector_mesh)
        connector_height = connector_max[2] - connector_min[2]
        connector_radius = max(math.hypot(x, y) for x, y, _ in connector_mesh.points)
        connector_inner_radius = min(
            math.hypot(x, y) for x, y, _ in connector_mesh.points
        )
        if abs(connector_min[2]) > 0.02 or abs(connector_height - args.base) > 0.02:
            raise ValueError(
                "Connector must span z=0 through the lamp base height; "
                f"found {connector_min[2]:.3f}..{connector_max[2]:.3f} mm"
            )
        outer_bottom_min_radius = min(
            math.hypot(x, y) for x, y, _ in outer_rings[0]
        )
        if connector_radius >= outer_bottom_min_radius:
            raise ValueError(
                "Connector does not fit inside the lamp's bottom profile: "
                f"{connector_radius:.3f} >= {outer_bottom_min_radius:.3f} mm"
            )
        try:
            connector_display = str(connector_path.relative_to(SCRIPT_DIR.parent))
        except ValueError:
            connector_display = str(connector_path)
        assembly_metadata = {
            "type": "overlapping multipart print assembly",
            "connector_source": connector_display,
            "connector_vertices": len(connector_mesh.points),
            "connector_faces": len(connector_mesh.faces),
            "connector_bounds_min_mm": connector_min,
            "connector_bounds_max_mm": connector_max,
            "connector_outer_diameter_mm": connector_radius * 2.0,
            "connector_minimum_radius_mm": connector_inner_radius,
            "placement": "centered at x=0, y=0; z=0..base height",
            "mates_with": "files/base/base-115mm-v3.stl",
        }
    ...
    basename = args.name or (
        ...
        f"{'_connector80' if connector_mesh is not None else ''}"
    )
    ...
    for output_format in args.formats:
        ...
        if output_format == "stl":
            write_binary_stl(
                path,
                combine_meshes([mesh, connector_mesh])
                if connector_mesh is not None
                else mesh,
            )
        elif output_format == "3mf":
            write_3mf(path, mesh, connector_mesh)
        elif output_format == "scad":
            write_scad(path, mesh, parameters, connector_mesh)
        outputs.append(path)
    ...
    if connector_mesh is not None:
        print(
            f"  Connector:             80 mm attach base "
            f"({len(connector_mesh.faces):,} faces), aligned at z=0..{args.base:.2f}"
        )
